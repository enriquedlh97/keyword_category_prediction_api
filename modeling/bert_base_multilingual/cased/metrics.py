...
def mean_auc_roc(predictions, labels, label_columns):
    ...
    for i, name in enumerate(label_columns):
        auc_roc_values = auroc(predictions[:, i], labels[:, i])
        auc_roc_dict[name] = auc_roc_values.reshape(-1).numpy()[0]
        auc_roc_values.append(auc_roc_values.reshape(-1).numpy()[0])
...
def mean_avg_precision(predictions, labels, label_columns):
    ...
    for i, name in enumerate(label_columns):
        ...
        avg_precision.append(avg_precision.reshape(-1).numpy()[0])

    return np.array(avg_precision).mean(), avg_precision_dict
