...
def model_evaluation(category, C, class_weight, solver, max_iter, warm_start, vectorizer_selection, strip_accents,
    ...
    for fold, (train_ids, test_ids) in enumerate(cv.split(pd_data)):
        ...
        model = LogisticRegression(C=C, class_weight=class_weight, solver=solver, max_iter=max_iter,
...
def get_solver(solver):
    ...
    if solver <= 1.0 / 4:
        return 'newton-cg'
    elif 2.0 / 4 >= solver > 1.0 / 4:
        return 'lbfgs'
    elif 3.0 / 4 >= solver > 2.0 / 4:
        return 'saga'
    elif 4.0 / 4 >= solver > 3.0 / 4:
        return 'sag'
